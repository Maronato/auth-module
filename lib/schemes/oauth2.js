...
export default class Oauth2Scheme {
  ...
  async mounted () {
    ...
    const expiration = this.$auth.syncTokenExpiration(this.name)
    const refresh = this.$auth.syncRefreshToken(this.name)
    if (expiration && Date.now() >= expiration && refresh) {
  ...
  async fetchUser () {
    ...
    if (!process.server) { this.$auth.setUser(user) } else {
      this.$auth.$storage.setState('loggedIn', Boolean(user))
    }
  }
  ...
  async _handleCallback (uri) {
    ...
    // token expiration
    let tokenExpiration = parsedQuery['expires_in']

    // token scope
    let tokenScope = parsedQuery['scope']
    ...
    if (this.options.response_type === 'code' && parsedQuery.code) {
      ...
      if (data.expires_in) {
        tokenExpiration = data.expires_in * 1000 + Date.now()
      }
    ...
    if (tokenExpiration) {
      this.$auth.setTokenExpiration(this.name, tokenExpiration)
    }

    // Store token scope
    if (tokenScope && tokenScope.length) {
      this.$auth.setTokenScope(this.name, tokenScope)
    }
