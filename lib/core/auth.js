...
export default class Auth {
  ...
  async request (endpoint, defaults) {
    ...
    const expiration = this.getTokenExpiration(this.strategy.name);
    const refresh_token = this.getRefreshToken(this.strategy.name);
    ...
    if (expiration && refresh_token && (now.getTime() / 1000 | 0) >= expiration) {
      ...
      let self = this;
      await this.ctx.app.$axios.request({
        method: 'post',
        ...
        const access_token = response.data.token_type + ' ' + response.data.access_token,
              ...
              expires = response.data.expires_in + (now.getTime() / 1000 | 0),
              scope = response.data.scope;

        self.setToken(strategy.name, access_token)
        self.setRefreshToken(strategy.name, refresh_token)
        self.setTokenExpiration(strategy.name, expires)
        self.setTokenScope(strategy.name, scope)

        strategy._setToken(access_token)
      })
    }
